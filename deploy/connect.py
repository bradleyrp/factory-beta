...
for connection_name,specs in sets.items():
	...
	for key,val in specs['paths'].items(): specs['paths'][key] = re.sub('NAME',specs['name'],val)
	
	#---make directories if they are absent
	root_data_dir = 'data/'+specs['name']
	...
	for key in ['post_plot_spot','post_data_spot','data_spots']:
