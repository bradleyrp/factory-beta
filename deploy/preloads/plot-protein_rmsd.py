#!/usr/bin/python -i

# plot prep
import os
if 'plotload' not in globals(): 
	execfile(os.environ['PYTHONSTARTUP'])
execfile('./omni/base/header.py')
from plotter import *
from base.store import plotload

# settings
plotname = 'protein_rmsd'
rmsd_bin_step = 0.1

# colors
import matplotlib.patheffects as path_effects
colormap = lambda i,n,name='jet': mpl.cm.__dict__[name](float(i)/n)

# load the upstream data
data,calc = plotload(plotname,work)

# prepare an axis
axes,fig = panelplot(
	layout={'out':{'grid':[1,1]},'ins':[{'grid':[1,2]}]},
	figsize=(12,8))

### PLOT

counter,xpos,xlim_left = 0,[],0
max_rmsd = max([max(data[sn]['data']['rmsds']) for sn in data])
rmsd_bins = arange(0,max_rmsd*1.1,0.1)
for snum,sn in enumerate(data):
	color = colormap(snum,len(data))
	# trajectory
	ax = axes[0]
	ts = array(data[sn]['slice']['timeseries'])
	rmsds = data[sn]['data']['rmsds']
	ts -= ts.min()
	# convert to ns
	ts = ts/1000.
	ax.plot(ts,rmsds,label=work.meta[sn]['name'],lw=2,color=color)
	ax.set_xlabel(r'time (ns)')
	# histograms
	ax = axes[1]
	counts,bins = histogram(rmsds,bins=rmsd_bins,normed=True)
	ax.fill_betweenx((bins[1:]+bins[:-1])/2.,counter,counter+counts,alpha=1.0,color=color,lw=0)
	# ax.axvline(counter,color='k',lw=2)
	if 0:
		mids = (bins[1:]+bins[:-1])/2.
		for ii,i in enumerate(mids[:-1]): 
			if counts[ii+1]!=0:
				ax.plot([counter_spacer*counter,counter_spacer*counter],[bins[ii],bins[ii+1]],
				color='k',lw=2,solid_capstyle='round')
	max_rmsd = max([max_rmsd,max(rmsds)])
	# compute drift
	m,b = polyfit(ts[len(rmsds)/10:],rmsds[len(rmsds)/10:],1)
	drift = m*(ts[-1]-ts[len(rmsds)/10])
	drift_start = ts[len(rmsds)/10]*m+b
	# drift arrows
	ax.arrow(counter,drift_start,0,drift,
		head_width=max(counts)*0.1,head_length=max_rmsd*0.05,fc=color,ec='w',lw=1.5,
		path_effects=[path_effects.Stroke(linewidth=4,foreground=color),path_effects.Normal()],zorder=3)
	xpos.append(counter)
	counter += max(counts)*1.1
	if snum==0: xlim_left = -1*max(counts)*0.1
ax.set_xticks(xpos)
ax.set_xticklabels([work.meta[sn]['name'] for sn in data],rotation=45,ha='right')
ax.set_xlim(xlim_left,counter)
for ax in axes: 
	ax.set_ylim(0,max_rmsd*1.1)
	ax.set_ylabel(r'RMSD $(\textrm{\AA})$')
ax.legend(loc='lower left')
picturesave('fig.%s'%plotname,work.plotdir,backup=False,version=True,meta={})
